...
### Automatic business mileage

The Mileage tab matches completed trips between three configured location zones (home, Miami client, Jersey client). Both endpoints must match exactly one zone; unknown, stale, overlapping or incomplete endpoints are not automatically classified. Passing a client on the road is not a match. Owner-configured matching routes are always Business; the destination client determines purpose on client-to-client trips. No per-trip approval is required.

Jersey's fixed purpose is `accounting, clerical, guidance, and technical work.` Miami uses commit titles and merged PR titles from the configured repositories and authors. The window runs after the previous recorded Miami visit's departure through the current visit's departure; the return-to-home leg shares the visit's purpose. An open visit is provisional. The first recorded visit cannot establish the prior visit boundary and remains purpose-pending. Missing or incomplete GitHub activity is flagged without changing the Business classification. Source titles are treated as data, not instructions; summaries are deterministic and cite the underlying activity rather than inventing tasks.

Configure addresses, coordinates, radii, repository names and GitHub usernames in Mileage → Places & rules (or authenticated POST `/api/mileage/settings`). All settings and private metadata remain outside Git. On this self-hosted deployment, the root-owned `tesla-mileage-github.timer` invokes `scripts/collect-github.mjs` hourly using the server's existing `gh` login. Install the service/timer from `deploy/`, run `systemctl daemon-reload`, then `systemctl enable --now tesla-mileage-github.timer`. It reads 90 days of default-branch commits and authored merged PRs, honors pagination limits, and sends only bounded source metadata to a bearer-authenticated collector endpoint. GitHub tokens never enter the frontend or Tesla service. A failed fetch does not publish a complete-coverage checkpoint. Additional branches and unattributed Git authors are not included.

Mileage reconstruction also runs in hourly app maintenance and from Refresh records. Existing history backfills two older days per sync plus the current and prior day; late telemetry updates cause affected days to be rescanned. Raw telemetry remains in D1. Derived mileage, location settings, source metadata, revision history, and immutable report snapshots live in the server's SQLite database and are included in its existing daily backup. These tables have no automatic expiry. Settings changes create a new version and recalculate current records; saved reports remain unchanged.

Monthly or annual date ranges are supported by the API (up to 370 days); the UI uses a month picker. Mileage report / Trip record saves an immutable snapshot, with Print / Save PDF and CSV exports. Reports expose estimated or unknown distances, unclassified trips, missing evidence and rule/source references. They are supporting mileage records, not payment receipts or tax determinations. Collection started partway through the year; full-year vehicle mileage requires odometer reconciliation. No tax rate or deduction amount is assumed.
